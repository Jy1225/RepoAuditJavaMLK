...
class DFBScanState(State):
    def __init__(self, src_values: List[Value], sink_values: List[Value]) -> None:
        ...
        # Responsibility transfer paths: src value -> {path_str -> path}
        self._transfer_paths: Dict[Value, Dict[str, List[Value]]] = {}
        ...
        self._transfer_paths_lock = threading.Lock()
    ...
    def update_transfer_paths(self, src_value: Value, path: List[Value]) -> None:
        """
        Update responsibility transfer paths
        """
        with self._transfer_paths_lock:
            if src_value not in self._transfer_paths:
                self._transfer_paths[src_value] = {}
            self._transfer_paths[src_value][str(path)] = path
    ...
    @property
    def transfer_paths(self) -> Dict[Value, Dict[str, List[Value]]]:
        """
        Get the responsibility transfer paths
        """
        with self._transfer_paths_lock:
            return self._transfer_paths.copy()
