...
class RepoAudit:
    def __init__(
        ...
        self.java_mlk_mode = args.java_mlk_mode
        ...
        if self.language == "Cpp":
            if Cpp_TSAnalyzer is None:
                raise ModuleNotFoundError("Cpp analyzer is unavailable in this build.")
            self.ts_analyzer = Cpp_TSAnalyzer(
                self.code_in_files, self.language, self.max_symbolic_workers
            )
        elif self.language == "Go":
            if Go_TSAnalyzer is None:
                raise ModuleNotFoundError("Go analyzer is unavailable in this build.")
            ...
            if Python_TSAnalyzer is None:
                raise ModuleNotFoundError("Python analyzer is unavailable in this build.")
    ...
    def validate_inputs(self) -> Tuple[bool, List[str]]:
        ...
        if self.args.scan_type == "dfbscan":
            ...
            requires_model = True
            if self.args.language == "Java" and self.args.bug_type == "MLK":
                requires_model = self.args.java_mlk_mode != "symbolic"
            if requires_model and not self.args.model_name:
...
def configure_args():
    ...
    parser.add_argument(
        "--java-mlk-mode",
        choices=["hybrid", "symbolic"],
        default="hybrid",
        help="Java MLK mode: hybrid (LLM + symbolic evidence) or symbolic (LLM-free).",
    )
    parser.add_argument(
