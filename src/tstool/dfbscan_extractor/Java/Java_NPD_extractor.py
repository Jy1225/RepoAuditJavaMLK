...
class Java_NPD_Extractor(DFBScanExtractor):
    def extract_sources(self, function: Function) -> List[Value]:
        ...
        source_bytes = self.ts_analyzer.fileContentBytes.get(file_path)
        if source_bytes is None:
            source_bytes = self.ts_analyzer.code_in_files[file_path].encode("utf-8")
            self.ts_analyzer.fileContentBytes[file_path] = source_bytes
