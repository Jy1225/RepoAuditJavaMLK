...
class TSAnalyzer(ABC):
    ...
    def __init__(
        ...
        # Cache utf-8 bytes for byte-accurate tree-sitter offsets.
        self.code_in_files_bytes: Dict[str, bytes] = {
            file_path: content.encode("utf-8")
            for file_path, content in code_in_files.items()
        }
        ...
        # Optional per-function metadata (language-specific).
        self.functionMeta: Dict[int, Dict[str, object]] = {}
        # Optional method signature index (language-specific).
        self.methodKeyToId: Dict[Tuple[str, str, int], Set[int]] = {}
        self.functionToFile: Dict[int, str] = {}
        self.fileContentDic: Dict[str, str] = {}
        self.fileContentBytes: Dict[str, bytes] = {}
        self.file_root_nodes: Dict[str, Node] = {}
    ...
    def _parse_single_file(self, file_path: str, source_code: str) -> Tuple[str, str]:
        ...
        try:
            ...
            exit(0)
        self.file_root_nodes[file_path] = tree.root_node
        if file_path not in self.fileContentBytes:
            self.fileContentBytes[file_path] = source_code.encode("utf-8")
    ...
    def _analyze_single_function(
        ...
        function_code = self.get_text_by_byte_range(
            file_name, function_node.start_byte, function_node.end_byte
        )
        meta = self.functionMeta.get(function_id, {})
        current_function = Function(
            ...
            method_sig=meta.get("method_sig", ""),
        )
    ...
    def parse_project(self) -> None:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for parse_future in concurrent.futures.as_completed(parse_futures):
                ...
                if file_path not in self.fileContentBytes:
                    self.fileContentBytes[file_path] = source.encode("utf-8")
                pbar.update(1)
            pbar.close()
    ...
    def extract_call_graph_edges(self, current_function: Function) -> None:
        ...
        for call_site_node in all_call_sites:
            ...
            if len(callee_ids) > 0:
                callsite_key = (call_site_node.start_byte, call_site_node.end_byte)
                current_function.callsite_callee_ids[callsite_key] = set(callee_ids)
    ...
    def get_output_value_at_callsite(
        ...
        """
        file_name = current_function.file_path
        name = self.get_text_by_byte_range(
            file_name, call_site_node.start_byte, call_site_node.end_byte
        )
        line_number = self.get_line_number_by_byte(file_name, call_site_node.start_byte)
        output_value = Value(
    ...
    def get_node_by_line_number(self, line_number: int) -> List[Tuple[str, Node]]:
        ...
        for function_id in self.function_env:
            ...
            for node in all_nodes:
                ...
                start_line = self.get_line_number_by_byte(file_name, node.start_byte)
                end_line = self.get_line_number_by_byte(file_name, node.end_byte)
    ...
    def _get_file_bytes(self, file_name: str) -> bytes:
        if file_name in self.fileContentBytes:
            return self.fileContentBytes[file_name]
        if file_name in self.code_in_files_bytes:
            data = self.code_in_files_bytes[file_name]
            self.fileContentBytes[file_name] = data
            return data
        if file_name in self.code_in_files:
            data = self.code_in_files[file_name].encode("utf-8")
            self.fileContentBytes[file_name] = data
            return data
        return b""

    def get_text_by_byte_range(
        self, file_name: str, start_byte: int, end_byte: int
    ) -> str:
        data = self._get_file_bytes(file_name)
        return data[start_byte:end_byte].decode("utf-8", errors="ignore")

    def get_line_number_by_byte(self, file_name: str, byte_offset: int) -> int:
        data = self._get_file_bytes(file_name)
